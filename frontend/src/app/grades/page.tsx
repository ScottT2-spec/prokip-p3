    ...
    <AppShell>
      ...
      <Modal open={defModalOpen} onClose={() => setDefModalOpen(false)} title={editingDef ? "Edit Grade Definition" : "Add Grade Definition"} maxWidth="max-w-xl">
        <form onSubmit={handleSaveDef} className="space-y-4">
          ...
          <div>
            <label className="input-label">Reward <span className="text-gray-400 font-normal">(what they earn at quarter end)</span></label>
            <textarea value={defForm.reward} onChange={(e) => setDefForm(p => ({ ...p, reward: e.target.value }))} className="input-field min-h-[70px] resize-none" placeholder="e.g. Quarterly bonus eligible, priority on projects..." />
          </div>
